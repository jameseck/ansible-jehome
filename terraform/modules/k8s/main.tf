...
# TODO: use count to create the resources - we should create the dhcp mappings in ansible so the ip's will already be dealt with
# ip = "${join(".", slice(split(".", local.starting_ip), 0, 3))}.${join("", slice(split(".", local.starting_ip), 3, 4)) + count.index}"

# TODO: turn user_data and network_data into template_file resources or use the template function
# TODO: pass root password hash in via variable
